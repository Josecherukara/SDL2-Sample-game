...
Music::Music(void) {
	...
	//vMusic.push_back(loadMusic("overworld-fast"));
        vMusic.push_back(loadMusic("batmanfull"));
	//vMusic.push_back(loadMusic("underground"));
	vMusic.push_back(loadMusic("coup"));
	...
	vChunk.push_back(loadChunk("catwomanpunch"));//coin"));
	...
	this->currentMusic = mOVERWORLDFAST;
}
...
void Music::changeMusic(bool musicByLevel, bool forceChange) {
    ...
    switch(WorldSubWorld)
    {
        case 0: eNew=mUNDERWATER;
                break;
        case 1: eNew=mOVERWORLD;
                break;
        case 2: eNew=mLOWTIME;
                break;
        case 11: eNew=mSTAR;
                break;
    }
